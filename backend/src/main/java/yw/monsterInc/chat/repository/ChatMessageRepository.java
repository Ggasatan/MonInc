package yw.monsterInc.chat.repository;
...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT DISTINCT c.sender FROM ChatMessage c " +
            "WHERE c.sender NOT IN :adminEmails " + // '관리자' 문자열 비교 대신 이메일 목록으로 비교
            "ORDER BY (SELECT MAX(c2.timestamp) FROM ChatMessage c2 WHERE c2.sender = c.sender) DESC")
    List<String> findAllDistinctUsersExcludingAdmins(@Param("adminEmails") List<String> adminEmails);
}
