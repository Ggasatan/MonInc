package yw.monsterInc.chat.service;
...
import yw.monsterInc.chat.repository.ChatMessageRepository;
...
public class ChatMessageService {
    ...
    public List<String> getAllChatUsers() {
        // 1. ADMIN 역할을 가진 사용자의 이메일 목록을 DB에서 조회
        List<String> adminEmails = memberRepository.findEmailsByMemberRole(MemberRole.ADMIN);

        // 2. 혹시 모르니 기존의 '관리자' 닉네임도 제외 목록에 추가
        //    (이렇게 하면 admin@test.com 과 '관리자' 둘 다 필터링 가능)
        if (adminEmails == null) {
            adminEmails = new ArrayList<>();
        }
        adminEmails.add("관리자");

        // 3. 관리자 이메일 목록을 쿼리 파라미터로 넘겨서 사용자 목록 조회
        return chatMessageRepository.findAllDistinctUsersExcludingAdmins(adminEmails);
    }
